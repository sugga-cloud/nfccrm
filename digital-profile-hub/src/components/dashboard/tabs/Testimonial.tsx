...
const TestimonialsTab = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Success", description: "Testimonial added!", className: "bg-brand-gold text-brand-dark" });
      setShowForm(false);
      setRating(5);
    },
    onError: () => {
    ...
    <div className="max-w-4xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-2 font-medium">Manage your social proof.</p>
        </div>
        <Button 
      ...
      {/* Create Form Section */}
      {showForm && (
        <Card className="mb-8 border-2 border-brand-gold/30 bg-white/5 shadow-xl shadow-brand-gold/5 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input name="reviewer_name" placeholder="e.g. John Doe" required className="rounded-lg border-white/10 bg-white/5" />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1 mt-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star 
                        key={star} 
                        className={`h-6 w-6 cursor-pointer transition-colors ${star <= rating ? 'text-brand-gold fill-brand-gold' : 'text-slate-500'}`}
              ...
              <div className="space-y-2">
                ...
                <Textarea name="content" placeholder="Paste the client's feedback here..." required rows={3} className="rounded-lg border-white/10 bg-white/5 resize-none" />
              </div>
              <div className="flex justify-end">
                <Button type="submit" disabled={createMutation.isPending} className="bg-brand-gold text-brand-dark rounded-xl px-8">
      ...
      {/* Stats Badge */}
      <div className="mb-4">
        <Badge variant="outline" className="text-brand-gold border-brand-gold/30 bg-brand-gold/10 px-4 py-1 rounded-full">
      ...
      <div className="space-y-4">
        {testimonials?.length === 0 && !showForm ? (
          ...
          testimonials?.map((t: any) => (
            <Card key={t.id} className={`group border-white/10 transition-all ${!t.is_visible ? 'opacity-60 grayscale bg-white/5' : 'bg-white/5'}`}>
              <CardContent className="p-6">
                <div className="flex flex-col md:flex-row justify-between gap-6">
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-2">
                      <div className="h-10 w-10 rounded-full bg-brand-gold/20 text-brand-gold flex items-center justify-center font-bold">
                        {t.reviewer_name.charAt(0)}
                      </div>
                      <div>
                        <h4 className="font-black italic uppercase text-white leading-tight">
                          {t.reviewer_name}
                        </h4>
                  ...
                  <div className="flex md:flex-col gap-2 justify-end">
                    <Button
                      ...
                      className={`rounded-xl border-white/10 flex items-center gap-2 ${t.is_visible ? 'hover:bg-white/10' : 'bg-brand-gold/10 text-brand-gold border-brand-gold/30'}`}
                    >
